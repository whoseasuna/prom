...
async def process_delay(message: Message, state: FSMContext):
    ...
    try:
        # Fetch dialogs first to populate the entity cache.
        # This is crucial for numeric chat IDs (supergroups) to be resolvable.
        try:
        ...
        for idx, group in enumerate(groups):
            # Ensure numeric IDs are int (already done in process_groups, but safe)
            target = group
